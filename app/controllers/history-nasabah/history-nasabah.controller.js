...
exports.getHstnb = async (req, res) => {
    ...
    try {
        ...
        var data = await hstnbRepo.gethistnbDB(condition, limit, offset);
        console.log("field >>>", field);

        const response = getPagingData(data, page, limit);
        let message = {
            english: `Successfully Retrieved Data History Transaksi Nasabah`,
            //"indonesia" : `Berhasil Mengambil Data EMP`,
        };
        res.send(jsonMessage.jsonSuccess(message, response));
    } catch (err) {
        const errMessage = err.message || "Some error occurred while get History Transaksi Nasabah";
